...
class ImageGeneratorService {
  ...
  String get _cfAccountId => dotenv.get('CLOUDFLARE_ACCOUNT_ID', fallback: '');
  String get _cfApiToken => dotenv.get('CLOUDFLARE_API_TOKEN', fallback: '');

  // ☁️ CLOUDINARY (Loaded from .env)
  String get _cloudinaryCloudName => dotenv.get('CLOUDINARY_CLOUD_NAME', fallback: '');
  String get _cloudinaryUploadPreset => dotenv.get('CLOUDINARY_UPLOAD_PRESET', fallback: '');

  // 🤖 CLOUDFLARE MODEL
  static const String _model = '@cf/black-forest-labs/flux-1-schnell';

  String get _cfEndpoint =>
  ...
  Future<Uint8List?> _generateWithCloudflare(String prompt) async {
    try {
      if (_cfApiToken.isEmpty || _cfAccountId.isEmpty) {
  ...
  Future<String> _uploadToCloudinary(Uint8List bytes, String title, String category) async {
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        addField('upload_preset', _cloudinaryUploadPreset);
